-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/pratyush/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/pratyush/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/pratyush/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/pratyush/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/pratyush/outputs' USING PigStorage('\t');
